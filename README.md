...
无需注册或配置密钥，使用 [Gold API 免费价格接口](https://gold-api.com/docs)的 `/price/XAU/CNY`（也支持 USD/HKD）。接口按金衡盎司报价，1 金衡盎司 = 31.1034768 克；系统换算为标的的报价单位，再按「重量 × 纯度 × 单价」估值，纯度只计算一次。该价格是国际黄金参考价，人民币和港币报价含供应商的汇率折算，不等同于上金所 Au99.99 成交价、金店零售价或回收价。

- 已有足金金条及以克记录的自定义实物黄金自动接入，无需重建持仓；未 seed 的数据库也会提供默认金条选项。
- 持有黄金且工作台页面可见时，每分钟尝试刷新；服务端报价缓存 60 秒并合并并发请求。后台常驻采集仍为每 15 分钟，无人访问的 Vercel 不持续采集。其他资产仍沿用各自缓存。
- 保存供应商真实 `updatedAt`，不以请求时间冒充行情时间。超过 1 小时未更新标为陈旧（休市时也可能出现）；接口失败保留原报价并显示失败原因，不填造新价格。
- 手动价格继续优先；自动价可用后，在资产详情切回自动模式。不支持的重量单位继续手动估值，防止千克与克混用。
- 免费服务的可用性和行情延迟取决于供应商，当前价格免费、无需 Key；不调用需要 Key 的历史接口。历史图仅展示实际采集记录。

### 长桥港股行情

1. 登录 [长桥开放平台](https://open.longportapp.com/)，开通 OpenAPI，在「用户中心 → 应用凭证」获取 **API Key 模式**的 App Key、App Secret、Access Token（不是 OAuth access token）。
2. 本地填入 `.env.local` 的 `LONGPORT_APP_KEY`、`LONGPORT_APP_SECRET`、`LONGPORT_ACCESS_TOKEN`，然后重启服务。Docker 部署填入 `.env.docker`，重新构建并启动 app：`docker compose --env-file .env.docker up -d --build app`。
3. 在资产页刷新行情。配置任意一项后港股走长桥，三项必须填完整；认证或行情权限失败会直接提示，不会静默改用其他来源。三项均为空时维持 Twelve Data。美股、A 股仍由 Twelve Data 报价，股票搜索也仍需 Twelve Data Key。

现有港股持仓无需重建，`09988`/`9988` 转为 `9988.HK`，`00700` 转为 `700.HK`。只读取行情与标的基础信息，不调用交易接口。报价保存供应商的实际时间，来源标注「行情可能延迟」，不保证实时；[OpenAPI 行情权限](https://open.longportapp.com/zh-CN/docs/quote/overview)与 App 行情权限独立。Token 过期后在开放平台更新环境变量并重启。API Key Access Token 默认有效期参见[官方快速开始](https://open.longportapp.com/zh-CN/docs/getting-started)。

长桥官方 Node SDK 使用原生模块，Next.js 已配置 `serverExternalPackages`；Docker 使用 Debian/glibc 镜像，支持官方 SDK 的 Linux x64/arm64 包，不需要额外网关服务。
...
**Vercel 免费模式**在打开工作台或返回标签页时尝试更新行情，数据库控制自动刷新至少间隔 15 分钟；持有实物黄金时缩短为 1 分钟，页面可见期间每分钟尝试刷新。无人访问时不持续采集。每天 Cron 按北京时间 08:00 的历史数据计算快照，实际触发可延迟，缺少历史价格不会回填未来报价。完整数据库备份由部署者在本机执行 `pnpm db:backup --env-file .env.backup`，不在 Vercel 写入备份。
